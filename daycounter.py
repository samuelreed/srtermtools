...
def parse_date(date_string: str) -> datetime.date:
    ...
    try:
        ...
        if parsed_date > max_date:
            print(
                f"Warning: Date '{date_string}' is more than 100 years in the future.",
                file=sys.stderr,
            )
        elif parsed_date < min_date:
            print(
                f"Warning: Date '{date_string}' is more than 100 years in the past.",
                file=sys.stderr,
            )
        ...
        if "time data" in str(e):
            print(
                f"Error: Invalid date format '{date_string}'. Please use YYYY-MM-DD format (e.g., 2023-12-25).",
                file=sys.stderr,
            )
        else:
...
def days_to_human_readable(start_date: datetime.date, end_date: datetime.date) -> str:
    """Convert date difference to human readable format."""
...
def format_day_phrase(days: int) -> str:
    """Format a day count with correct singular or plural grammar"""
    day_label = "day" if days == 1 else "days"
    return f"{format_days_count(days)} {day_label}"


def process_date_calculation(
    mode: str,
    ...
    if mode == "countdown":
        ...
        if days_diff < 0:
            ...
            if use_human_readable:
                ...
                print(f"{event_description} was " f"{format_day_phrase(abs_days)} ago.")
        else:
            ...
            if use_human_readable:
                ...
                print(
                    f"It is {format_day_phrase(days_diff)} until "
                    f"{event_description}."
                )
        ...
        if days_diff < 0:
            ...
            if use_human_readable:
                ...
                print(
                    f"{event_description} will be in " f"{format_day_phrase(abs_days)}."
                )
        else:
            # Event is in the past
            if use_human_readable:
                ...
                print(
                    f"It has been {format_day_phrase(days_diff)} since "
                    f"{event_description}."
                )
...
if __name__ == "__main__":
    ...
    if args["countdown"]:
        process_date_calculation(
            "countdown", eventdate, event_description, use_human_readable
        )
    elif args["countup"]:
        process_date_calculation(
            "countup", eventdate, event_description, use_human_readable
        )
    else:
